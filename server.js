...
function displayHomePage(request,response){
  ...
  .then(() => {
    ...
    .then(sqlQueryResult =>{
      users = sqlQueryResult.rows;
    }).catch(error => handleError(error,response));
  }).catch(error => handleError(error,response));
  superagent.get(url)
  .then(secondaryResult =>{
    var entries = secondaryResult.body;
    var quote = getRandomQuote(new createQuoteList(entries));
    var ejsObject = {hpElements:{quote, users}};
    response.render("./index.ejs",ejsObject);
  }).catch(error => handleError(error,response))
};
...
function addToBoard(req, res) {
  const sqlStr = 'INSERT INTO visions(image_url,username,author,author_url, description) VALUES($1,$2,$3,$4,$5) RETURNING id;';
  const sqlArr = [req.body.image, req.body.username, req.body.image_author, req.body.image_author_url, req.body.description];
  client.query(sqlStr,sqlArr)
  .then(result =>{
...
function handleError(error,res) {
  console.log("Unable to fulfill the request: ",error.message);
...
try{
    ...
    .then( () => {
      ...
      .catch(err => console.log("This this isn't working: ",err));
    }catch(error){
    handleError(error);
    }
//#endregion
